...
func NewHSL(h,s,l float64) HSLA {
   hsla := HSLA{H: h, S: s, L: l, A: 1}
   // fmt.Println(hsla.ToHex())
   return  hsla
}
...
func (rgba RGBA) GetCC() *CoolorColor {
   return NewCoolorColor(rgba.ToHex())
}
...
func (hsla HSLA) GetCC() *CoolorColor {
   return hsla.ToRGBA().GetCC()
}

func (hsla HSLA) ToHex() string {
   return hsla.ToRGBA().ToHex()
}
