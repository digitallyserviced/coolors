...
func NewCoolorColorsPaletteSwatch(
	...
	ccs.Table.SetSelectedStyle(
		tcell.StyleDefault.Foreground(0).Background(tcell.Color238),
	)
...
// func NewShortcutHandler(s *shortcuts.Scope) *ShortcutHandler {
// 	sh := &ShortcutHandler{
// 		Scope: s,
// 	}
// 	return sh
...
func (ccs *CoolorColorSwatch) InputHandler() func(event *tcell.EventKey, setFocus func(p tview.Primitive)) {
	...
	ccs.UpdateView()
      // MainC.PageStack.R
			MainC.PageStack.RouteInput(event, setFocus, ccs.Table.InputHandler())
