...
var (
	SubShortcuts   *ScriptShortcuts
	SuperShortcuts *ScriptShortcuts
	keyLabel       []rune
	subLabel       []rune
)

var (
	superShortTexts []rune
	superShortSubs  []rune
)
...
func (sss *ScriptShortcuts) NewScriptShortcut(text, sub rune) ScriptShortcut {
	ss := &ScriptShortcut{
		text:      text,
		script:    sub,
		wrapBegin: sss.wrapBegin,
		wrapEnd:   sss.wrapEnd,
	}
...
func (sss *ScriptShortcuts) TakeNext() ScriptShortcut {
	avail := sss.Avail()
	if len(avail) == 0 {
		return sss.NewScriptShortcut(rune(0), rune(0))
	} else {
...
func (ssss *ScriptShortcuts) MapTxtShort() {
	for i, v := range ssss.origText {
		...
		sss := ssss.NewScriptShortcut(v, sub)
